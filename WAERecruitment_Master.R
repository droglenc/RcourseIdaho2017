...
WalleyeEL2 <- filterD(WalleyeEL,age5<2500)

SVi2 <- srStarts(age0~age5,data=WalleyeEL2,type="independence",plot=TRUE)
SVbh2 <- srStarts(age0~age5,data=WalleyeEL2,type="BevertonHolt",plot=TRUE)
SVr2 <- srStarts(age0~age5,data=WalleyeEL2,type="Ricker",plot=TRUE)
SVs2 <- srStarts(age0~age5,data=WalleyeEL2,type="Shepherd",plot=TRUE)
SVsl2 <- srStarts(age0~age5,data=WalleyeEL2,type="SailaLorda",plot=TRUE)

# Fit using levenberg-marquardt algorithm (less sensitive to starts)
SRi.fit2 <- nlsLM(logage0~log(SRi(age5,a)),data=WalleyeEL2,start=SVi2)
SRbh.fit2 <- nlsLM(logage0~log(SRbh(age5,a,b)),data=WalleyeEL2,start=SVbh2)
SRr.fit2 <- nlsLM(logage0~log(SRr(age5,a,b)),data=WalleyeEL2,start=SVr2)
SRs.fit2 <- nlsLM(logage0~log(SRs(age5,a,b,c)),data=WalleyeEL2,start=SVs2)
SRsl.fit2 <- nlsLM(logage0~log(SRsl(age5,a,b,c)),data=WalleyeEL2,start=SVsl2)

# Compare models
ms2 <- list(SRi.fit2,SRbh.fit2,SRr.fit2,SRs.fit2,SRsl.fit2)
mnames <- c("Independence","Beverton-Holt","Ricker","Shepherd","Saila-Lorda")
aictab(ms2,modnames=mnames)

# Examine model fits ... ugh
clrs <- c("black","black","red","blue","orange")
plot(age0~age5,data=WalleyeEL2,pch=19,
     xlim=c(0,3500),ylim=c(0,40000),
     xlab="Number of Age-5 and Older",ylab="Number of Age-0")
curve(SRi(x,coef(SRi.fit2)),from=0,to=4000,add=TRUE,lty=2,col=clrs[1])
curve(SRbh(x,coef(SRbh.fit2)),from=0,to=4000,add=TRUE,lwd=2,col=clrs[2])
curve(SRr(x,coef(SRr.fit2)),from=0,to=4000,add=TRUE,lwd=2,col=clrs[3])
curve(SRs(x,coef(SRs.fit2)),from=0,to=4000,add=TRUE,lwd=2,col=clrs[4])
curve(SRsl(x,coef(SRsl.fit2)),from=0,to=4000,add=TRUE,lwd=2,col=clrs[5])
legend("topright",mnames,col=clrs,lty=c(2,1,1,1,1),lwd=c(1,2,2,2,2),
       bty="n",cex=0.9)
# --- ASSIGNMENT END ---------------------------------------------------
...
makeStudentScript("WAERecruitment")
